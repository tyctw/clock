...
const Details: React.FC<{ exam: Exam }> = ({ exam }) => <dl className="grid grid-cols-2 gap-x-4 gap-y-4 px-5 py-5 text-sm">
  {[['國文（含寫作）', exam.chinese], ['英語／英聽', exam.english], ['數學', exam.math], ['自然', exam.science]].map(([name, value]) => <div key={String(name)}><dt className="text-xs font-black text-[#8793a5]">{name}</dt><dd className="mt-1 font-bold leading-6 text-[#3d5272]">{value}</dd></div>)}
  <div className="col-span-2"><dt className="text-xs font-black text-[#8793a5]">社會</dt><dd className="mt-1 font-bold text-[#3d5272]">{exam.social}</dd></div>
  {exam.note && <div className="col-span-2 border-t border-[#eee4d5] pt-4"><dt className="text-xs font-black text-[#8793a5]">備註</dt><dd className="mt-1 font-bold text-[#3d5272]">{exam.note}</dd></div>}
</dl>;
...
export const MockExamSchedulePage: React.FC = () => {
  ...
  return <section className="animate-fade-in pb-2 pt-6 sm:pb-6 sm:pt-12">
    ...
    <div className="mt-7 rounded-[1.75rem] border border-[#e6dccc] bg-[#fffdfa] p-4 shadow-[0_8px_20px_rgba(72,53,27,0.06)] sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-black text-[#183468]">選擇日程／版本</p><p className="mt-1 text-xs font-medium text-[#78869a]">{publisher.description}</p>{publisher.officialUrl && <a href={publisher.officialUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-xs font-black text-[#fa6841] transition-colors hover:text-[#c94b2c]">查看 {publisher.name} 官方範圍表 <ExternalLink className="h-3.5 w-3.5" /></a>}</div><div ref={versionSelectorRef} className="-mx-1 flex max-w-full gap-1.5 overflow-x-auto rounded-2xl bg-[#f5f0e7] p-1.5 scrollbar-thin"><span className="sr-only">滑動以選擇日程或版本</span>{publishers.map((item, index) => <button key={item.name} data-version-index={index} type="button" onClick={() => setSelected(index)} aria-pressed={selected === index} className={`shrink-0 whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-black transition-all ${selected === index ? 'bg-[#183468] text-white shadow-sm' : 'text-[#66758a] hover:text-[#183468]'}`}>{item.name}</button>)}</div></div></div>
    <div className="mt-6 space-y-4 md:hidden">{publisher.exams.map(exam => <article key={`${exam.code}-${exam.label}`} className="overflow-hidden rounded-3xl border border-[#e6dccc] bg-[#fffdfa] shadow-[0_8px_20px_rgba(72,53,27,0.07)]"><div className="flex items-center justify-between bg-[#f5f0e7] px-5 py-4"><div className="flex items-center gap-3"><span className="flex min-w-9 items-center justify-center rounded-xl bg-[#183468] px-2 py-2 text-xs font-black text-white">{exam.code}</span><div><p className="text-sm font-black text-[#183468]">{exam.label}模擬考</p><p className="mt-0.5 text-xs font-bold text-[#718099]">{exam.date.join('、')}</p></div></div><span className="rounded-full bg-[#ffe9bd] px-3 py-1.5 text-xs font-black text-[#9a6714]">{publisher.name}</span></div><Details exam={exam} /></article>)}</div>
    <div className="mt-6 hidden overflow-hidden rounded-[1.75rem] border border-[#e6dccc] bg-[#fffdfa] shadow-[0_12px_30px_rgba(72,53,27,0.08)] md:block"><div className="border-b border-[#eee4d5] px-6 py-5 sm:px-8"><div className="flex items-center gap-2 text-sm font-bold text-[#536987]"><BookOpen className="h-4 w-4 text-[#fa6841]" /> {publisher.name}｜各科範圍依施測次別整理</div></div><div className="overflow-x-auto"><table className="min-w-[980px] w-full text-left"><thead className="bg-[#f5f0e7] text-xs font-black tracking-wide text-[#435b7e]"><tr><th className="px-6 py-5">回次</th><th className="px-5 py-5">施測日期</th><th className="px-5 py-5">國文<br /><span className="font-medium">（含寫作）</span></th><th className="px-5 py-5">英語／英聽</th><th className="px-5 py-5">數學</th><th className="px-5 py-5">自然</th><th className="px-5 py-5">社會</th><th className="px-5 py-5">備註</th></tr></thead><tbody className="divide-y divide-[#eee4d5] text-sm font-semibold text-[#3d5272]">{publisher.exams.map(exam => <tr key={`${exam.code}-${exam.label}`} className="transition-colors hover:bg-[#fff7e9]"><td className="px-6 py-6"><span className="inline-flex min-w-10 justify-center rounded-xl bg-[#183468] px-3 py-2 font-black text-white">{exam.code}</span><div className="mt-2 text-xs font-bold text-[#66758a]">{exam.label}</div></td><td className="whitespace-nowrap px-5 py-6 leading-7 text-[#183468]">{exam.date.map(day => <div key={day}>{day}</div>)}</td><td className="px-5 py-6">{exam.chinese}</td><td className="px-5 py-6">{exam.english}</td><td className="px-5 py-6">{exam.math}</td><td className="px-5 py-6 leading-6">{exam.science}</td><td className="px-5 py-6">{exam.social}</td><td className="px-5 py-6 text-xs leading-6 text-[#66758a]">{exam.note ?? '—'}</td></tr>)}</tbody></table></div></div>
    <div role="note" className="mt-5 flex items-center justify-center gap-2 rounded-2xl border border-[#f2c86e] bg-[#fff4d9] px-4 py-3 text-center text-sm font-black text-[#82570d]"><CircleAlert className="h-4 w-4 shrink-0" /><span>實際施測日期、範圍與安排，仍需依照各校規定為主。</span></div>
  </section>;
};
